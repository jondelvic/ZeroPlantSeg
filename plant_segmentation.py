...
def plant_segmentation(bgr_img_dir, masks_dirs: list, config_path, output_dir, strip=".png", GENERATE_NEW_CLUSTER=True, VALIDATION_MODE=False):
    ...
    for masks_dir in tqdm(masks_dirs):
        ...
        np.save(os.path.join("./",img_name),clustering_points)
        # clustering
...
def setup_args():
    ...
    parser.add_argument("--dataset", type=str, help="dataset name", choices={'phenobench', 'growliflower', 'sb20'}, default='phenobench')
...
if __name__ == "__main__":
   ...
   # bgr_img_dir = os.path.join('/datasets', args.dataset, args.mode, 'images')
    ...
    #np.save("./wo_pp.npy",wo_pp,allow_pickle=True)
    #np.save("./w_pp.npy",w_pp,allow_pickle=True)